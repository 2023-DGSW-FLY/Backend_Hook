package com.innosync.hook.jwt;

import com.innosync.hook.config.ExpireTime;
...
public class JwtTokenProvider {
    ...
    public JwtTokenProvider(@Value("${oauth.jwt.secret}") String secretKey) {
        ...
        // Ensure the key length is sufficient (at least 256 bits or 32 bytes)
        if (keyBytes.length < 32) {
            throw new IllegalArgumentException("JWT HMAC-SHA key length is not secure enough.");
        }
